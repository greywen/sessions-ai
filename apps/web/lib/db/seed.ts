...
async function seed() {
  const databaseUrl = process.env.DATABASE_URL || 'postgresql://postgres:123456@localhost:5432/llm_sessions';
  ...
  const adminEmail = process.env.ADMIN_EMAIL || 'admin@llm-sessions.local';
